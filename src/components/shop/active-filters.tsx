    ...
    <div className="mb-6 flex flex-wrap items-center gap-2">
      ...
      {filters.map((filter) => (
        <Link
          ...
          scroll={false}
          prefetch={false}
          className="ui-mono group inline-flex items-center gap-2 rounded-full border border-rule py-1.5 pl-3.5 pr-2.5 transition-colors hover:border-oxblood hover:bg-oxblood hover:text-paper"
        >
          <span className="sr-only">Remove filter </span>
          <span className="text-oxblood transition-colors group-hover:text-paper/75">
            {`${filter.group}:`}
          </span>
      ...
      <Link
        href={clearHref}
        scroll={false}
        className="ui-mono ml-1 text-oxblood underline decoration-1 underline-offset-4 transition-colors hover:text-ink"
      >
        Clear all
      </Link>
    </div>
