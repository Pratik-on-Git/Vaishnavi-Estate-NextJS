...
import FilterPanel, { type BrowseItem, type PanelGroup } from "./filter-panel";
import Pagination from "./pagination";
import SortMenu from "./sort-menu";

/**
 * The browse surface: the whole catalogue, one collection, or a search - one
...
export default async function ShopView({
  ...
  basePath: string;
  /** Absent on the all-products page. */
  ...
  // A phone shows fewer per page: the same 24 cards are a far longer scroll in
  // one column than in four, and the pagination control is what a shopper
  // reaches for instead.
      ...
      <div className="shell pb-6 pt-8 md:pt-12">
        ...
        {query ? (
          <p className="ui-mono mt-5">
            <span className="text-oxblood">Results for </span>
            <span className="font-bold">&ldquo;{query}&rdquo;</span>
            <Link
              href={shopUrl(basePath, params, { q: null })}
              className="ml-3 text-oxblood underline decoration-1 underline-offset-4 hover:text-ink"
            >
              Clear search
            </Link>
          </p>
            ...
            {results.items.length ? (
              <>
                <Grid className="grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                  <ProductGridItems products={results.items} sizes={GRID_SIZES} />
                </Grid>
                <Pagination
                  page={results.page}
              ...
              <div className="rounded-plate bg-tint px-8 py-20 text-center">
                <Headline as="p" size="md">
                ...
                <p className="body-mono mx-auto mt-4 max-w-measure">
                  {hasActiveFilters(state)
                    ...
                    : query
                      ? "Try a broader term - a roast, a grind or a collection."
                      : "Everything in this collection is on its way. Explore the rest of the catalogue."}
                </p>
                <Link
