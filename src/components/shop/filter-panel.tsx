 ...
 * Nothing here knows what a grind or a roast is.
 */
    ...
    <details open={defaultOpen} className="group rule-b py-5">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 [&::-webkit-details-marker]:hidden">
        ...
        <span className="flex items-center gap-2">
          ...
          <ChevronDownIcon
            aria-hidden
            className="h-4 w-4 shrink-0 text-oxblood transition-transform duration-200 group-open:rotate-180"
          />
        </span>
      </summary>
...
function ValueRow({ value }: { value: PanelValue }) {
  const body = (
    <>
      <span
        aria-hidden
        className={clsx(
          "flex h-4 w-4 shrink-0 items-center justify-center rounded-[0.2rem] border transition-colors",
          value.active
            ? "border-ink bg-ink text-paper"
            : "border-rule group-hover/row:border-oxblood"
        )}
      >
        {value.active ? <CheckIcon className="h-3 w-3" strokeWidth={3} /> : null}
      </span>
      <span className="min-w-0 flex-1 break-words">{value.label}</span>
      <span className="spec-mono shrink-0 tabular-nums">{value.count}</span>
    </>
  );
  ...
  if (!value.active && value.count === 0) {
    return (
      <li>
        <span className="ui-mono flex items-start gap-2.5 py-1.5 text-oxblood opacity-45">
          {body}
        </span>
      </li>
    );
    ...
    <li>
      <Link
        ...
        className={clsx(
          ...
          value.active ? "text-ink" : "text-oxblood hover:text-ink"
        )}
      >
        {body}
      </Link>
  ...
  return (
    <>
      ...
      {hidden.length ? (
        ...
        >
          <summary className="ui-mono mt-1.5 inline-flex cursor-pointer list-none items-center gap-1.5 text-oxblood underline decoration-1 underline-offset-4 hover:text-ink [&::-webkit-details-marker]:hidden">
